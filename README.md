...
| 经配对的工作台访问 | 以 agent 实际开放的能力为准；远程对话文本不能代替本人确切审批；已配对的审批方法可在 Web 核对并回答具体问题 |

当前还没有接入日历写入、支付或任意电脑操作。个人协作模式也不提供自动后台唤醒。你现有 agent 的其他工具权限仍由它原来的运行环境管理。

新版工作台可将真实回合内准备的任务、审批与协作关联到原对话，并把回合结束作为安全结果提醒。
`completed` 表示模型回合结束，业务完成仍以真实协作结果为准。仅升级服务端不会自动升级用户设备上的 connector；
旧配对保留原方法范围，旧记录不猜测来源。细节见 [对话来源与结果提醒](python/README.md#对话中的事项来源与结果提醒)。
