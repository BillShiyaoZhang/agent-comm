...
- On pre-v2 helpers, `POST /api/v1/mq/store` returned HTTP 202, `success:true` and a stable `message_id` after local durable acceptance. The new helper requires policy setup and an explicit v2 business route, as described below. Workers retry the same signed ciphertext. `platform_queued` means the platform accepted it; neither state proves recipient execution or task completion.
...
The Agent-to-Agent v2 route is `POST /api/v2/mq/store`. It requires a policy signing root and platform PeerID provisioned outside the platform. The v0.9.0 bundle can verify a peer key from an exact URN on one Platform; the older v0.8.0 bundle additionally requires independently verified, manually pinned full peer Ed25519 keys. A verified v2 installation bundle can call `v2-ensure-policy-root` before daemon startup; exact repeat pins are accepted without rewriting the old record, while a different root or platform fails closed. The helper runs a signed ephemeral X25519 handshake through the platform, then uses session-derived private message keys or two HPKE CEK slots for compliance. The receiver verifies the signed admission receipt and CEK proof before inbox commit and platform ACK. `GET /api/v2/disclosure` exposes the public root loaded by the running daemon and separates signed policy facts, including gateway key and platform decryptability, from local consent; unknown values are null. Default local disclosure is private only. `v2-allow-compliance` requires the exact verified policy hash, and any new epoch or key needs a fresh owner decision. `v2-disallow-compliance` stops later compliance work but cannot retract already disclosed plaintext. The same local inbox exposes verified `mode`, `policy_epoch`, gateway key ID and envelope hash per message. See [the v2 protocol reference](../architecture/PROTOCOL_V2.md) and [helper commands](../../references/helper-api-en.md).

V2 uses separate SQLite outbox, handshake and session tables. Persist `mailbox.db` and its WAL files across restarts. On a policy epoch change the helper quarantines old immutable ciphertext and ends the old session. The new helper's ordinary `/api/v1/mq/store` fails with a machine-readable `policy_root_required` (428), `consent_required` (403), or `upgrade_required` (409); it never silently converts that request. Updated Python, Hermes, and OpenClaw business senders inspect disclosure and use v2 when ready. It disables direct legacy message streams while v2 is enabled. Paired Web control responses use only the explicit `/api/v1/managed/mq/store` route and the platform still checks an active managed certificate; they must not be presented as v2 privacy or compliance evidence. Publish a signed private compatibility policy and distribute its trusted root before deploying this new helper; unmodified old binaries remain a temporary v1 compatibility concern.
