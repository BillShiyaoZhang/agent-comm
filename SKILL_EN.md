...
The install/pairing script does not pin the v2 policy root or platform PeerID, and it does not authorize Agent-to-Agent compliance disclosure. A new helper without that pin rejects ordinary sends with HTTP 428 `policy_root_required`. The deployer must obtain and verify the root public key and platform PeerID through a channel independent of the platform, then run `v2-pin-policy-root <keys_dir> <root_public_key_hex> <expected_platform_peer_id> <independent_verification_note>`. Do not use the platform's own bootstrap response as verification. Old binaries can send only during a signed `private` policy's `allow_v1=true` compatibility period. Successful Web pairing only grants its managed control path.
