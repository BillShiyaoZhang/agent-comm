...
Hermes personal collaboration and remote workspace access are separately enabled through `collaboration_enabled` and `remote_enabled`. Pairings constrain the console URN, allowed methods and expiry. Remote requests cannot impersonate native owner approval. Native confirmation checks the actual owner conversation, current turn and the answer to the specific question. A contact entry or an `allow_from` value alone does not confer owner control.
...
- [Architecture background in Chinese](../OVERVIEW.md) and [English](../OVERVIEW_EN.md)
