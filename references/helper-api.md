...
入站的 `sender_urn` 已通过传输验证，保留关联字段。先去重、处理或写入持久队列，再 ACK 本机 helper。平台 ACK 由 helper 在验证/解密并持久保存 inbox 后处理。不要因 SSE 收到、开始处理、`Last-Event-ID` 或模型调用返回就提前确认。状态与崩溃恢复细节见 [通信合同](../docs/HERMES_INTEGRATION.md)。
