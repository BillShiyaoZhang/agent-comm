...
| `release_assets.py` | Validate release tags and source pins, assemble tested client artifacts, and publish only after draft uploads pass checksum verification |
...
The Release workflow first tests the SDK, then builds four helpers and two wheels. It requires a deployment checkout whose committed SDK reference matches the release tag, and reuses that checkout's complete-bundle builder. For v2 bundles, assembly reads `tools/release/trust/<tag>.json` from the deployment commit itself, passes those exact reviewed public bytes to the builder, then verifies all four ZIPs carry identical bytes covered by their `SHA256SUMS.json`. The early-access and GitHub manifests record the trust-file and root-key SHA256 fingerprints. Publication creates a new draft, verifies every uploaded asset, rechecks the remote tag, and only then publishes that draft. Existing releases are never overwritten. Run publication and downloader checks with `python -m unittest discover -s tools -p 'test_release*.py'`.
