...
The Release workflow first tests the SDK, then builds four helpers and two wheels. It requires a deployment checkout whose committed SDK reference matches the release tag, and reuses that checkout's complete-bundle builder. Publication creates a new draft, verifies every uploaded asset, rechecks the remote tag, and only then publishes that draft. Existing releases are never overwritten. Run publication and downloader checks with `python -m unittest discover -s tools -p 'test_release*.py'`.
