...
本页索引 v1 实现。v1 字段和编号以 `proto/*.proto` 为准；独立 v2 JSON 协议见[隐私与合规信封](PROTOCOL_V2.md)。修改 `.proto` 后必须同步生成 Go 文件并验证客户端/服务端兼容性。
...
- helper 的 v1 HTTPS MQ 使用静态 X25519 信封，`dr` 是独立的状态化 P2P 协议。v2 使用临时握手与显式模式策略；三者的密钥和状态生命周期不能混写。
